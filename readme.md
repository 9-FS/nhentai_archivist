...
- `CIRCUMVENT_LOAD_BALANCER`, optional, defaults to `false`
    ...
    Setting this to `true` circumvents the load balancer at `i.nhentai.net` and directly uses a random media server, for example `i2.nhentai.net`. Only use this if the load balancer is broken. You can find this out by setting `DEBUG = true` and checking the logs for consistent download errors when using `i.nhentai.net`.
