    ...
    'program: loop // keep running for server mode
    {
        'iteration: // particular iteration of gathering id to download and downloading, client mode does only does 1 iteration, server mode unlimited
        {
            ...
            for (i, hentai_id) in hentai_id_list.iter().enumerate()
            {
                ...
                match Hentai::new(*hentai_id, &db, &http_client, NHENTAI_HENTAI_SEARCH_URL, &config.LIBRARY_PATH, config.LIBRARY_SPLIT.unwrap_or_default(), &config.TITLE_TYPE).await // create hentai, use u32 with 0 to disable library split and not Option<u32> with None, because that would make Some(0) an invalid state
                {
